...
class Orders:
    ...
    def create(**kwargs):
        ...
        if order is None:
            raise InvalidArgumentException('`order_id` is a required argument for Order.create()\n')
    ...
    def status(**kwargs):
        order = util.get_arg(kwargs,'order_id')
        if order is None:
            raise InvalidArgumentException('`order_id` is a required argument for Order.status()\n')
    ...
    def update(**kwargs):
        ...
        if order_id is None:
            raise InvalidArgumentException('`order_id` is a required argument for Order.update()\n')
