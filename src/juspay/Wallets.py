...
class Wallets:
    ...
    def create(**kwargs):
        ...
        if customer_id is None or gateway is None:
            raise InvalidArgumentException('`customer_id` & `gateway` are required arguments for Wallets.create()\n')
    ...
    def create_and_authenticate(**kwargs):
        ...
        gateway = util.get_arg(kwargs, 'gateway')
        if customer_id is None or gateway is None:
            raise InvalidArgumentException('`customer_id` & `gateway` are required arguments for '
    ...
    def list(**kwargs):
        ...
        if customer_id is None and order_id is None:
            raise InvalidArgumentException('`customer_id` or `order_id` is a required argument for Wallets.list()\n')
    ...
    def refreshBalance(**kwargs):
        ...
        if customer_id is None:
            raise InvalidArgumentException('`customer_id` is a required argument for Wallets.refreshBalance()\n')
    ...
    def authenticate(**kwargs):
        ...
        if wallet_id is None:
            raise InvalidArgumentException('`wallet_id` is a required argument for Wallets.authenticate()\n')
    ...
    def refresh_by_wallet_id(**kwargs):
        wallet_id = util.get_arg(kwargs, 'wallet_id')
        if wallet_id is None:
            raise InvalidArgumentException('`wallet_id` is a required argument for Wallets.refresh()\n')
    ...
    def link(**kwargs):
        ...
        if wallet_id is None or otp is None:
            raise InvalidArgumentException('`wallet_id` & `otp` are required arguments for Wallets.link()\n')
    ...
    def delink(**kwargs):
        wallet_id = util.get_arg(kwargs, 'wallet_id')
        if wallet_id is None:
            raise InvalidArgumentException('`wallet_id` is a required argument for Wallets.delink()\n')
