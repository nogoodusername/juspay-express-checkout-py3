class JuspayException(Exception):
    ...
    def __str__(self):
        if self._message is not None and self.http_status is not None:
            error_response = u"ERROR - Request failed with httpResponseCode : {0}.\nerrorMessage : {1}".format(self.http_status, self._message)
        elif self._message is not None:
            error_response =  u"ERROR - Request failed. Message : {0}".format(self._message)
        elif self.json_body is not None:
            error_response = u"ERROR - Request failed with httpResponseCode : {0}\n{1}".format(self.http_status, self.json_body)
        else:
            error_response = u"ERROR - Request failed with unknown reason, please contact support@juspay.in."
...
class InvalidArgumentException(JuspayException):
    def __init__(self, message=None):
        super(InvalidArgumentException, self)\
            .__init__(message=message or "Please pass required parameters.")
