...
def var_dump(obj, depth=4, l=""):
    ...
    if isinstance(obj, dict):
        ...
        canprint = lambda o: isinstance(o, (int, float, bytes, str, bool, type(None), types.LambdaType))
        try:
    ...
    for k, v in objdict.iteritems():
        if v is not None and v != '':
            non_null_dict[k] = v
    return "\n<" + name + ">" + "\n{\n" + "\n".join(l + repr(k) + ": " + var_dump(v, depth=depth - 1) + "," for k, v in
                                                    non_null_dict.iteritems()) + "\n" + l + "}\n"
